...
import psycopg2 
import hashlib
import random  # 랜덤 메시지 뽑기를 위한 부품 추가

# ==========================================
# 💌 선생님 전용: 응원 메시지 보관함 (언제든 자유롭게 수정/추가하세요!)
# ==========================================
...
st.markdown("""
    ...
    .question-text { font-size: 1.5rem !important; font-weight: 800; line-height: 1.8 !important; color: #1F2937; margin-bottom: 20px; text-align: center; }
    ...
    label { line-height: 1.6 !important; font-size: 1.2rem !important; color: #374151; font-weight: 600; }
...
if st.session_state.login_user_id is not None:
    ...
    if st.sidebar.button("로그아웃"):
        # 로그아웃 시 설문 상태도 초기화하여 다음 학생이 1번 문항부터 시작하도록 함
    ...
    with center_col:
        with st.container(border=True):
            ...
            if st.button("🚪 학생 로그인 / 최초 등록", type="primary"):
                if not s_name or not s_password: st.error("❌ 이름과 비밀번호를 모두 입력해 주세요.")
                    ...
                    if user is None:
                        try:
                            cursor.execute("INSERT INTO users (role, grade, room, number, name, password_hash) VALUES ('student', %s, %s, %s, %s, %s)", (s_grade, s_room, s_number, s_name, hashed_pw))
                            conn.commit()
                        ...
                        if user[2] == hashed_pw:
                            if user[1] != s_name: st.error(f"❌ 등록된 학생 이름과 다릅니다.")
                            else:
    ...
    if st.session_state.needs_password_change:
        ...
        # 🌟 카드형 설문 진행 화면
            ...
            with main_col:
                ...
                # 3. 포켓몬 카드 UI 영역
                with st.container(border=True):
                    st.markdown(f"<div style='text-align: center; font-size: 5rem; margin-bottom: 20px;'>{data['icon']}</div>", unsafe_allow_html=True)
                    st.markdown(f"<p class='question-text'>{data['question']}</p>", unsafe_allow_html=True)
                    
                    # 기존에 선택한 값이 있으면 유지 (뒤로 가기 방어 로직)
                ...
                with col_next:
                    if step < total_steps - 1:
                        ...
                        if st.button("📊 제출하기", type="primary", use_container_width=True):
                            # 최종 점수 계산
                            raw_score = sum(SURVEY_DATA[k]["weight"] * st.session_state.survey_responses.get(k, 0) for k in SURVEY_DATA)
                            ...
                            # 제출 완료 상태로 변경 후 새로고침 (결과화면 띄우기)
                            st.session_state.survey_completed = True
                            st.rerun()

# ==========================================
# 🧑‍🏫 5. 교사용 채널 (이전 3구간 로직 유지)
# ==========================================
        ...
        with col_dash1:
            ...
            for _, row in df_merged.iterrows():
                ...
                if pd.isna(sc): st.text(f"• {w}주차 월요일: 미입력")
                    ...
                    elif sc >= 0.9: status = "🟠 주의군 (정기 관찰 필요)"
                    else: status = "🟢 안정군 (일상 유지)"
            ...
            if st.button("🔄 이 학생의 비밀번호를 '12345678'로 강제 초기화"):
                ...
                conn.commit()
                conn.close()
                st.success("비밀번호가 '12345678'로 변경되었습니다.")
